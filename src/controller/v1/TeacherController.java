...
//			}
	...
	public void getList() {
		...
		try {
			if(key != null) {
				String sql1="SELECT count(*),a.*,b.name as clazz_name from teacher a LEFT JOIN clazz b  ON a.clazz_id=b.id where a.name LIKE '%"+key+"%'";
				String sql="SELECT count(*),a.*,b.name as clazz_name from teacher a LEFT JOIN clazz b  ON a.clazz_id=b.id where a.name LIKE '%"+key+"%' limit "+limit;
				...
				String sql="SELECT a.*,b.name as clazz_name from teacher a LEFT JOIN clazz b  ON a.clazz_id=b.id limit "+limit;
				ArrayList<HashMap<String, String>> list = M("teacher").query(sql);
				for(int i=0;i<list.size();i++) {
					if(list.get(i).get("isadmin").equals("0")) {
						list.get(i).put("isadmin", "普通教工");
	...
	public void do_edit_info() {
		...
		tea.put("tid", tid);
		tea.put("isadmin",isadmin);
		tea.put("original_pwd",origial_pwd);
		user.put("name", tea_name);
		user.put("state", "2");
		user.put("update_time", TimeUtil.getLongTimeStamp() + "");
		try {
